...
return	$res=	$this->db->get()->result();
	...
	public function deletefromsub_cat($varcat)
	{
		foreach($varcat as $cats)
		for($i=0;$i<count($cats);$i++)
		{
			$this->db->select('id');
			$this->db->from('sub_category');
		$this->db->where('category_id',$cats['id']);
	$result[]=	$this->db->get()->result_array();
	
			//echo '<br>';$result[$i];
		$this->db->where_in('category_id',$cats['id']);
	...
	public function deletfaq($varsucat)
	{
		print_r($varsucat);
		foreach($varsucat as $rs)
		print_r($rs);
		for($i=0;$i<count($rs);$i++)
		{
			//echo '<br>';$result[$i];
		$this->db->where('subcategory_id',$rs['0']['id']);
		$this->db->delete('faq');
	...
	public function deletfaq_old($post)
	{
	...
	$this->db->where('id',$post['catid']);
	$this->db->delete('faq');
	$this->db->where('faq_id',$post['catid']);
...
{	
	...
	public function subcategorysss($catid)
	{
		return	$this->db->select('*')
		          ->from('category')
	...
	public function updatedepartkment($post)
	{
	  $this->db->set('department',$post['uid']);
     $this->db->where('ticket_id',$post['tid']);
       $this->db->update('create_ticket');	
	}
	public function savedeptranferdata($post,$userid)
	{
		$dataArray=array(
		"ticket_id"=>$post['tid'],
		"user_id"=>$userid,
		"old_department"=>$post['dep'],
				);
				$this->db->insert('dept_tranfer',$dataArray);
				return true;
